...
```{r setup, include = FALSE}
  ...
  library(papaja); library(stargazer); library(kableExtra)
...
```
A total of `r n` protocols were collected **(n=`r n`)**, administered to a number of `r n.male` *males* (`r n.male.prc `%) and `r n.female` *females* (`r n.female.prc`%, see Table \@ref(tab:gender)), most of them being *university graduates* (`r n.st.lic`, representing `r n.st.lic.prc`%), followed by *high school graduated* (`r n.st.hsc`, representing `r n.st.hsc.prc`%) and the persons with *master's degrees* (`r n.st.mst`, representing `r n.st.mst.prc`%). The research group also contains graduates of *post-secondary schools* (`r n.st.phs`, representing `r n.st.phs.prc`%) and graduates of *arts and crafts schools* (`r n.st.art`, representing `r n.st.art.prc`%), the other categories being much less represented.

```{r gender, echo=FALSE}
  knitr::kable(gender$output.table, format = "latex", digits = 2, booktabs = T,
                       caption = "Participants's gender", escape = F)
```
```{r education, echo=FALSE}
   knitr::kable(studies$output.table, format = "latex", digits = 2, booktabs = T,
                       caption = "Participants's educational level", escape = F)
```
Remarcăm, totuși, numărul mare al persoanelor care nu au precizat nivelul de studii (`r n.nost`, reprezentând `r n.nost.prc`%), acestea urmând a fi eliminate din analizele ce presupun utilizarea acestei variabile (see Table \@ref(tab:education)).

În privința modalității de administrate, un număr de `r n.paper` protocoale au fost administrate în *formă creion-hârtie (`r n.paper.prc`%)*, `r n.online` protocoale fiind colectate direct *online (`r n.online.prc`%)* (see Table \@ref(tab:gather))
```{r gather, echo=FALSE}
   knitr::kable(online$output.table, format = "latex", digits = 2, booktabs = T,
                       caption = "Type of data gathering", escape = F)
```
